...
# Crea rete solo con archi positivi
edges_pos <- E(g)[E(g)$weight > 0]
g_pos <- subgraph.edges(g, edges_pos, delete.vertices = FALSE)

# Crea rete solo con archi negativi (valore assoluto)
...
# Combina i risultati
nodes <- V(g)$name  # o 1:vcount(g) se non hai nomi
...
results <- data.frame(
  node = V(g)$name,  # o 1:vcount(g)
  pagerank_pos = pr_positive,
  pagerank_neg = pr_negative,
  pagerank_net = pr_combined,
  katz = V(g)$Katz
)

# Visualizza i top nodi
results %>% 
  arrange(desc(pagerank_net)) %>% 
  head(10)
